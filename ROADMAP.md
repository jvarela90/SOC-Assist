...
| # | Feature | Estado |
|---|---------|--------|
| 107 | **R1** Rate limiting en `/chatbot/session/answer` (20 req/min por IP, reutiliza middleware existente) | ✅ |
| 108 | **R2** Cleanup automático de sesiones chatbot huérfanas >24 h al arrancar (`scheduler.cleanup_orphaned_sessions`) | ✅ |
| 109 | **R3** Paginación 50 registros/página en `/incidentes` (offset/limit DB-level, controles Bootstrap) | ✅ |
| 110 | **N1** Exportación chatbot a PDF/Imprimir — endpoint `/chatbot/session/{uuid}/print` + template standalone | ✅ |
| 111 | **N2** Exportación chatbot a JSON y CSV — endpoint `/chatbot/session/{uuid}/export?format=json\|csv` | ✅ |
| 112 | **N3** 2FA TOTP (pyotp) — setup con QR code, activar/desactivar, flujo login en dos pasos (`/verify-2fa`) | ✅ |
| 113 | **N4** Integración TheHive v5 — exportar incidente como Case con observables; panel config `/admin/thehive` | ✅ |
| 114 | **N5** API Tokens Bearer — modelo `APIToken`, bcrypt-hash con prefix, CRUD admin `/admin/api-tokens` | ✅ |
| 115 | **P1** Modo simulacro en chatbot — toggle UI, sesiones de test que no crean incidentes reales | ✅ |

### Nuevos archivos/rutas (Fase 12)

| Archivo/Ruta | Descripción |
|---|---|
| `app/services/thehive.py` | Cliente async TheHive v5 (casos + observables) |
| `app/templates/api_tokens.html` | Gestión de API Tokens (crear, revocar, ver prefix) |
| `app/templates/thehive_config.html` | Configuración de integración TheHive |
| `app/templates/chatbot_print.html` | Página imprimible standalone del resultado de sesión |
| `app/templates/totp_setup.html` | Setup 2FA: QR code (qrcodejs CDN), clave manual, confirmación |
| `app/templates/verify_2fa.html` | Pantalla de verificación TOTP en login |
| `GET /chatbot/session/{uuid}/print` | PDF/Print de sesión chatbot |
| `GET /chatbot/session/{uuid}/export` | Export JSON/CSV de sesión chatbot |
| `GET/POST /cuenta/2fa` | Setup 2FA del usuario actual |
| `GET/POST /verify-2fa` | Verificación TOTP (paso 2 del login) |
| `GET /admin/api-tokens` | Listado de tokens API del usuario |
| `POST /admin/api-tokens/create` | Crear token Bearer |
| `POST /admin/api-tokens/{id}/revoke` | Revocar token |
| `GET/POST /admin/thehive` | Configuración de TheHive |
| `POST /incident/{id}/export-thehive` | Exportar incidente a TheHive como Case |
...
└── app/
    ...
    ├── services/
    │   ├── thehive.py        # Cliente TheHive v5: export_incident_to_thehive()
